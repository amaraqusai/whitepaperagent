...
A **~15-page academic-grade Hebrew/English bilingual whitepaper** generated end-to-end by a **CrewAI** multi-agent pipeline.
This repository contains the LaTeX source, compiled PDF, Python visualization scripts, research dossiers, and the CrewAI orchestration codebase that powers the agent team.
...
| Feature | Description |
|:--------|:------------|
| **Multi-Agent CrewAI Pipeline** | 5 specialized AI agents (Researcher → Data Scientist → Writer → Typesetter → Editor) using CrewAI framework |
...
whitepaperagent/
│
├── agent_system.py              # 🤖 CrewAI multi-agent pipeline (5 agents)
├── requirements.txt             # 📦 Python dependencies (crewai, dotenv, etc.)
├── .env                         # 🔑 API keys (gitignored)
├── TODO.md                      # 📝 Project task list & remaining work
│
├── latex/                       # 📄 All LaTeX source & compiled output
│   ├── main.tex                 #     Master LaTeX source (~17-page bilingual paper)
│   ├── main.pdf                 #     Final compiled PDF output
│   ├── references.bib           #     BibTeX academic citation database (10 entries)
│   ├── figures/
│   │   ├── soc_control_room.png #     🖼️ SOC control room image
│   │   ├── anomalies_over_time.pdf  # 📈 LLM vs. Heuristic time-series graph
│   │   └── performance_plot.pdf     # 📊 F1-Score vs. Model Scale graph
│   └── scripts/
│       ├── generate_graph.py        # F1-Score vs. Model Scale plot generator
│       └── generate_graph_anomalies.py  # LLM vs. Heuristic time-series plot
│
├── scripts/
│   └── run_quality_checks.py    # ✅ Automated project quality & compliance checker
│
├── output/                      # 📂 Agent intermediate outputs (Markdown → LaTeX)
│   ├── research_brief.md        #     Agent 1 output
│   ├── math_and_plots.md        #     Agent 2 output
│   ├── article_draft.md         #     Agent 3 output (Markdown draft)
│   ├── draft_main.tex           #     Agent 4 output
│   └── polished_main.tex        #     Agent 5 output
│
├── AGENT_MASTER_CHECKLIST.md    # ✅ 400+ line execution checklist (10 phases)
├── AGENT_TASKS.md               # 📋 Task assignments for each agent role
├── PROJECT_PLAN.md              # 📐 System architecture & development phases
├── RESEARCH_NOTES.md            # 🔬 Academic research dossier with statistics
├── OUTLINE.md                   # 🗂️ Chapter structure & page budgets
├── AI_WORKFLOW_LOG.md           # 🤖 AI pipeline evolution & prompt engineering log
├── COST_PRICING_ANALYSIS.md     # 💰 Token usage, API cost analysis & scaling
├── UI_UX_SHOWCASE.md            # 🖥️ CLI interface & PDF visual layout documentation
├── EXTENSIBILITY_GUIDE.md       # 🔌 Guide to adding agents & swapping LLM backends
└── README.md                    # 📖 This file
```
...
The whitepaper is generated by a team of **5 specialized CrewAI agents**, each with a distinct `role`, `goal`, and `backstory`. The agents are defined in [`agent_system.py`](agent_system.py) and execute sequentially using `Process.sequential`:

### Agent 1: Lead Academic Researcher
- **Input:** Project outline (`OUTLINE.md`) + research notes
- **Task:** Gathers quantitative cybersecurity statistics, threat landscape data, and compiles BibTeX references
- **Output:** `output/research_brief.md`

### Agent 2: AI Security Data Scientist
- **Input:** Research brief from Agent 1 (via `context`)
- **Task:** Formulates Bayesian threat scoring math, derives economic cost optimization, writes matplotlib scripts
- **Output:** `output/math_and_plots.md`

### Agent 3: Senior Technical Writer (Markdown)
- **Input:** Research brief + Data Scientist output (via `context`)
- **Task:** Writes the complete ~15-page article in **Markdown** format (Hebrew with English terms)
- **Output:** `output/article_draft.md` — Intermediate Markdown for review

### Agent 4: LaTeX Typesetter & Document Engineer
- **Input:** Markdown draft from Agent 3 (via `context`)
- **Task:** Converts Markdown to compilable XeLaTeX with TikZ, tabularx, amsmath, fancyhdr, biblatex
- **Output:** `output/draft_main.tex`

### Agent 5: Senior Technical Editor & QA Engineer
- **Input:** Draft LaTeX from Agent 4 (via `context`)
- **Task:** Validates environments, BiDi rendering, TikZ syntax, hyperref citations, and compilation readiness
- **Output:** `output/polished_main.tex` — Final bug-free compilable LaTeX
...
```
               ...
               │  "Math models & plots"  │
               └────────────┬────────────┘
                            │ output/math_and_plots.md
                            ▼
               ┌─────────────────────────┐
               │  Agent 3: MD Writer     │
               │  "Markdown article"     │
               └────────────┬────────────┘
                            │ output/article_draft.md  ◄── Review checkpoint
                            ▼
               ┌─────────────────────────┐
               │  Agent 4: LaTeX Writer  │
               │  "Convert MD → .tex"    │
               └────────────┬────────────┘
                            │ output/draft_main.tex
                            ▼
               ┌─────────────────────────┐
               │  Agent 5: QA Editor     │
               │  "Validate & polish"    │
               └────────────┬────────────┘
                            │ output/polished_main.tex
                            ▼
               ┌─────────────────────────┐
               │  XeLaTeX + Biber        │
               │  (4-pass compilation)   │
...
Or install manually:
```bash
pip install crewai crewai-tools python-dotenv matplotlib numpy
```
...
This creates `figures/performance_plot.pdf` and `figures/anomalies_over_time.pdf` inside the `latex/` folder.
...
| **Agent Orchestration** | **CrewAI** framework with `Process.sequential` |
| **LLM Model** | Gemini 2.0 Flash via LiteLLM (`gemini/gemini-2.0-flash`) |
| **API Key Management** | `python-dotenv` (.env file, gitignored) |
